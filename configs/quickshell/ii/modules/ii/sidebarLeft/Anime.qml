...
Item {
    ...
    property var aniListResults: []
    property bool loadingAniList: false
    ...
    function handleInput(inputText) {
        if (inputText.startsWith(root.commandPrefix)) {
            ...
            if (Persistent.states.booru.category === "anime" || Persistent.states.booru.category === "manga") {
                root.loadingAniList = true;
                root.aniListResults = [];
                Booru.clearResponses(); // Clear booru responses to avoid mixed views
                try {
                    AniList.search(inputText, Persistent.states.booru.category.toUpperCase(), (results) => {
                        root.aniListResults = results;
                        root.loadingAniList = false;
                        if (results.length === 0) {
                            Booru.addSystemMessage("No results found on AniList for: " + inputText);
                        }
                    });
                } catch (e) {
                    console.error("Failed to call AniList service:", e);
                    Booru.addSystemMessage("Error: Could not access AniList service.");
                    root.loadingAniList = false;
                }
            } else {
                // Create tag list
    ...
    ColumnLayout {
        ...
        Item {
            ...
            StyledListView { // Booru responses
                ...
                model: ScriptModel {
                    values: (Persistent.states.booru.category === "anime" || Persistent.states.booru.category === "manga") ? root.aniListResults : root.responses
                }
                delegate: Loader {
                    width: booruResponseListView.width
                    sourceComponent: (Persistent.states.booru.category === "anime" || Persistent.states.booru.category === "manga") ? aniListDelegate : booruDelegate
                ...
                onDragEnded: { // Pull to load more
                    if (Persistent.states.booru.category === "wallpaper") {
            ...
            PagePlaceholder {
                id: placeholderItem
                z: 2
                shown: (Persistent.states.booru.category === "wallpaper" ? root.responses.length === 0 : root.aniListResults.length === 0) && !root.loadingAniList
                icon: Persistent.states.booru.category === "wallpaper" ? "bookmark_heart" : (Persistent.states.booru.category === "anime" ? "movie" : "book")
                title: Persistent.states.booru.category.charAt(0).toUpperCase() + Persistent.states.booru.category.slice(1)
                description: ""
            ...
            MaterialLoadingIndicator {
                ...
                loading: root.pullLoading || Booru.runningRequests > 0 || root.loadingAniList
        ...
        Rectangle { // Tag input area
            ...
            RowLayout { // Controls
                ...
                Item {
                    id: categorySwitcher
                    implicitWidth: categoryIndicator.implicitWidth
                    implicitHeight: categoryIndicator.implicitHeight
                    property bool menuOpen: false
                    z: 1 // Ensure it's above other controls in the row

                    ApiInputBoxIndicator {
                        id: categoryIndicator
                        icon: Persistent.states.booru.category === "wallpaper" ? "image" : (Persistent.states.booru.category === "anime" ? "movie" : "book")
                        text: {
                            const cat = Persistent.states.booru.category;
                            return cat.charAt(0).toUpperCase() + cat.slice(1);
                        }
                        tooltipText: Translation.tr("Click to change category")
                    }

                    MouseArea {
                        anchors.fill: parent
                        onClicked: categorySwitcher.menuOpen = !categorySwitcher.menuOpen
                    ...
                    Rectangle {
                        id: categoryMenu
                        visible: categorySwitcher.menuOpen
                        width: 120
                        ...
                        ColumnLayout {
                            ...
                            Repeater {
                                model: [
                                    { name: "Anime", icon: "movie", value: "anime" },
                                    { name: "Manga", icon: "book", value: "manga" },
                                    { name: "Wallpaper", icon: "image", value: "wallpaper" }
                                ]
                                delegate: RippleButton {
                                    Layout.fillWidth: true
                                    implicitHeight: 35
                                    buttonRadius: Appearance.rounding.smaller
                                    colBackground: Persistent.states.booru.category === modelData.value ? Appearance.colors.colSecondaryContainer : "transparent"
                                    ...
                                    onClicked: {
                                        Persistent.states.booru.category = modelData.value;
                                        categorySwitcher.menuOpen = false;
                                    }
                                }
                            }
                        }
                ...
                /*
                StyledText {
                    visible: Persistent.states.booru.category === "wallpaper"
